...
- Bro Detail composer mic input in push-to-talk mode bypasses Communication Brain and Draft Brain after local recording; clients upload raw audio, the selected Bro's executor node transcribes it with local Whisper, and Newbro turns the resulting text into direct Codex work, starting a direct task from idle or queueing the transcript behind the active Codex turn; direct audio marks the task to suppress Communication Brain notification candidates

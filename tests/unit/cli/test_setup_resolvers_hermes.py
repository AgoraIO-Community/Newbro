...
from newbro.cli.setup_resolvers import SetupResolutionCallbacks, executor_runtime_ready, resolve_executor_setup_values


def _make_ready_callbacks(*, hermes_on_path: bool = True) -> SetupResolutionCallbacks:
    """Minimal callbacks for executor_runtime_ready tests (generic/hermes branch)."""
    return SetupResolutionCallbacks(
        connector_config_path=lambda: pathlib.Path(tempfile.gettempdir()) / "hermes-ready-config.yaml",
        ...
        prompt_text_value=lambda label, default_value="", required=False: default_value,
        prompt_executor_selection=lambda default_selected=None: ["hermes"],
        detected_codex_command=lambda: None,
        command_available=lambda command: command == "hermes" if hermes_on_path else False,
        resolve_agora_connector_setup_values=lambda *args, **kwargs: ({}, {}),
    )


def test_hermes_ready_with_no_configured_command_when_on_path():
    assert executor_runtime_ready("hermes", existing_block={}, existing_values={}, callbacks=_make_ready_callbacks(hermes_on_path=True)) is True


def test_hermes_not_ready_when_not_on_path():
    assert executor_runtime_ready("hermes", existing_block={}, existing_values={}, callbacks=_make_ready_callbacks(hermes_on_path=False)) is False

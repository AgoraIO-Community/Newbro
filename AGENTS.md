...
- **Do not break the codex multi-message turn contract.** A single codex turn streams several `agentMessage` items tagged with a `phase`: `commentary` items are intermediate working narration (reasoning steps) and only `final_answer` is the settled answer. This was regressed repeatedly; the invariants are non-negotiable: (1) commentary never settles the turn — it settles exactly once on `final_answer` / outbound `codex_turn_event` `completed` / `turn/completed`, with no `completed → running` flicker; (2) commentary never fills the answer slot (`turn.assistant`), only `final_answer`/phase-less native answers do; (3) the final answer is not also recorded as a reasoning step; (4) the UI shows the streaming commentary as a prominent line that collapses into a compact step list, with the final answer as the answer. Owners: `_merge_timeline_turn` + `apply_codex_thread_timeline_event` (`src/newbro/runtime/bro_detail_thread_*`), `_record_native_turn_reasoning` (`src/newbro/runtime/session.py`), and `splitLiveSteps` + `buildTurnRenderModel` + `LiveTurnBubble` (`clients/web/src/`). Any change touching phase handling, turn settling, answer/step routing, or the bubble split MUST keep `tests/unit/runtime/test_codex_multi_message_turn.py` (real-wire replay), the `selected_codex_thread` / `commentary` / `_merge_timeline_turn` tests in `tests/unit/runtime/test_session_runtime.py`, and `clients/web/src/lib/splitLiveSteps.test.ts` green, and re-verify against the fixture `docs/protocol/fixtures/codex-multi-message-turn-sample.jsonl`. See `docs/protocol/codex-turn-streaming.md`.
